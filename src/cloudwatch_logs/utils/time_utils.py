...
def convert_time_to_timestamp(time_str: str) -> int:
    ...
    Raises:
        ...
        TypeError: time_strがNoneまたは文字列でない場合
    ...
    """
    if time_str is None:
        raise TypeError("time_str cannot be None")

    if not isinstance(time_str, str):
        raise TypeError("time_str must be a string")

    # Z suffix を +00:00 に変換 (Python 3.10 の fromisoformat は Z を完全サポートしていない)
    ...
    try:
        ...
        raise ValueError(f"Invalid ISO format string: '{time_str}': {str(e)}")
