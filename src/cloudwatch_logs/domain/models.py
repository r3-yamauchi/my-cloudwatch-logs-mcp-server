...
class SavedLogsInsightsQuery(BaseModel):
    ...
    def model_dump(self, **kwargs) -> Dict[str, Any]:
        """モデルを辞書に変換し、セットをリストに変換する."""
        data = super().model_dump(**kwargs)
        # セットをリストに変換
        if 'logGroupNames' in data:
            data['logGroupNames'] = list(data['logGroupNames'])
        if 'logGroupPrefixes' in data:
            data['logGroupPrefixes'] = list(data['logGroupPrefixes'])
        return data
...
class QuerySyntaxElement(BaseModel):
    """クエリ構文要素の詳細情報."""

    name: str = Field(..., description='構文要素の名前')
    element_type: str = Field(..., description='要素のタイプ（command, function, operator等）')
    description: str = Field(..., description='構文要素の説明')
    syntax: Optional[str] = Field(default=None, description='構文の書式')
    examples: List[Dict[str, Any]] = Field(default_factory=list, description='使用例のリスト')
    limitations: List[str] = Field(default_factory=list, description='制限事項のリスト')
    related_elements: List[str] = Field(default_factory=list, description='関連する構文要素のリスト')


class QuerySyntaxDocumentation(BaseModel):
    """クエリ構文ドキュメントの検索結果."""

    query_type: str = Field(..., description='クエリの種類（overview, command, function, search等）')
    content: Dict[str, Any] = Field(..., description='ドキュメントの内容')
    matched_elements: List[QuerySyntaxElement] = Field(
        default_factory=list, description='検索にマッチした構文要素のリスト'
    )
    total_elements: int = Field(default=0, description='検索結果の総数')
    search_metadata: Optional[Dict[str, Any]] = Field(
        default=None, description='検索に関するメタデータ'
    )
